...
现在我需要一个功能，推文内容发给genimi之后，gemini会分析整篇推文的内容然后生成一个十五个字以内的标题用来插入notion数据库的“标题”字段，而不是现在把推文的内容当做标题。并且我已经在notion数据库中添加了一个新的property叫“总结”，genimi需要将整篇推文总结成一小段文字然后插入到这个字段，大概150字以内，实际长度根据推文内容来决定。
